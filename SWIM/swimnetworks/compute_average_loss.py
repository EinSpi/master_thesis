...
if __name__ == "__main__":
    ...
    with open("Results/"+experiment_name+"/"+"obj_"+objective+"/"+"act_"+act+"/fdbk_%d/layers%d/width%d/errors.txt" % (feed_back,l,w), "r") as file:
        mse_values = []
        rel_l2_values = []
    ...
    print("Average MSE: %f,Average REL: %f" % (avg_mse, avg_rel_l2))

    # Write the result to a new file
    with open("Results/"+experiment_name+"/"+"obj_"+objective+"/"+"act_"+act+"/fdbk_%d/layers%d/width%d/avg_error.txt" % (feed_back,l,w), "w") as output_file:
