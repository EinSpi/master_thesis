...
def neural_net(X, weights, biases, ratweightsP, ratweightsQ):
    ...
    for l in range(0, num_layers - 2):
        ...
        H = tf.math.divide(tf.math.polyval(ratweightsP[(degP + 1) * l:(degP + 1) * l + (degP + 1)], H),
                           tf.math.polyval(ratweightsQ[(degQ + 1) * l:(degQ + 1) * l + (degQ + 1)], H))
    W = weights[-1]
    b = biases[-1]
...
class DeepHPM:
    def __init__(self, t_train, x_train, u_train, t_val, x_val, u_val, u_layers, lb_idn, ub_idn, rP, rQ, l, w):
        ...
        self.idn_init(t_train, x_train, u_train, t_val, x_val, u_val, u_layers, rP, rQ, l, w)
    ...
    def idn_init(self, t_train, x_train, u_train, t_val, x_val, u_val, u_layers, rP, rQ, l, w):
        ...
        # Type not implemented 
            ...
            raise ValueError(
                "Initialization coefficients for type (%d, %d) rationals not implemented. Run the MATLAB code initial_rational_coeffs.m to find the coefficients.")
    ...
    def save_loss(self):
        ...
        np.savetxt("GD_Results/Rational/layers%d/width%d/loss_rat.csv" % (self.l, self.w), L, delimiter=',')

        # Validation loss
        L = np.vstack((its, self.ValArray)).transpose()
        np.savetxt("GD_Results/Rational/layers%d/width%d/val_rat.csv" % (self.l, self.w), L, delimiter=',')
...
if __name__ == "__main__":
    ...
    if not os.path.exists("GD_Results/Rational/layers%d/width%d" % (l, w)):
        os.makedirs("GD_Results/Rational/layers%d/width%d" % (l, w))
    ...
    # Model
    model = DeepHPM(t_train, x_train, u_train, t_val, x_val, u_val, u_layers, lb_idn, ub_idn, rP, rQ, l, w)
    ...
    #write mse
    with open("GD_Results/Rational/layers%d/width%d/errors.txt" % (l,w), "w") as f:
        f.write("MSE: %e" % error_u_identifier)
    print('wrote GD err to file')
    #writing coeffs after gd
    print("ratweightsP after training")
    optimized_rat_weightsP = [model.sess.run(p) for p in model.ratweightsP]
    print(optimized_rat_weightsP)
    for i in range(len(optimized_rat_weightsP)):
        if i == 0:
            with open("GD_Results/Rational/layers%d/width%d/rat_coeffsP.txt" % (l,w), "w") as f:
                f.write(f"{optimized_rat_weightsP[i][0]:.6f}\n")
        else:
            with open("GD_Results/Rational/layers%d/width%d/rat_coeffsP.txt" % (l,w), "a") as f:
                f.write(f"{optimized_rat_weightsP[i][0]:.6f}\n")

    print("ratweightsQ after training")
    optimized_rat_weightsQ = [model.sess.run(q) for q in model.ratweightsQ]
    print(optimized_rat_weightsQ)
    for i in range(len(optimized_rat_weightsQ)):
        if i == 0:
            with open("GD_Results/Rational/layers%d/width%d/rat_coeffsQ.txt" % (l,w), "w") as f:
                f.write(f"{optimized_rat_weightsQ[i][0]:.6f}\n")
        else:
            with open("GD_Results/Rational/layers%d/width%d/rat_coeffsQ.txt" % (l,w), "a") as f:
                f.write(f"{optimized_rat_weightsQ[i][0]:.6f}\n")

    U_pred = griddata(X_idn_star, u_pred_identifier.flatten(), (T_idn, X_idn), method='cubic')

    # Save identifier as CSV file
    np.savetxt("GD_Results/Rational/layers%d/width%d/idn_rat.csv" % (len(u_layers)-2, u_layers[1]), U_pred, delimiter=',')
    ...
    savefig("GD_Results/Rational/layers%d/width%d/KdV_idn_rat" % (l,w))
