...
U_tanh = np.loadtxt("Results/tanh/idn_tanh.csv", delimiter=',')
U_adaptive_tanh=np.loadtxt("Results/adaptive_tanh/idn_adaptive_tanh.csv", delimiter=',')
...
E_tanh = np.mean((Exact_idn-U_tanh)**2)
E_adaptive_tanh =np.mean((Exact_idn-U_adaptive_tanh)**2)
print("Error ReLU = %e, Error Sine = %e, Error Rat = %e, Error tanh = %e, Error adaptive_tanh = %e" % (E_relu, E_sine, E_rat, E_tanh, E_adaptive_tanh))
...
fig.colorbar(h, cax=cax)
ax.set_title("Rat Error")

######## tanh error #######################
gs.update(top=0.8, bottom=0.2, left=0.1, right=0.9, wspace=0.5)
ax = plt.subplot(gs[0, 3])
h = ax.imshow(abs(Exact_idn-U_tanh), interpolation='nearest', cmap='jet', 
              extent=[lb_idn[0], ub_idn[0], lb_idn[1], ub_idn[1]],
              origin='lower', aspect='auto', vmin=0.0, vmax=0.01)
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
...
fig.colorbar(h, cax=cax)
ax.set_title("tanh Error")

######## adaptive_tanh error #######################
gs.update(top=0.8, bottom=0.2, left=0.1, right=0.9, wspace=0.5)
ax = plt.subplot(gs[0, 4])
h = ax.imshow(abs(Exact_idn-U_adaptive_tanh), interpolation='nearest', cmap='jet',
              extent=[lb_idn[0], ub_idn[0], lb_idn[1], ub_idn[1]],
              origin='lower', aspect='auto', vmin=0.0, vmax=0.01)
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
